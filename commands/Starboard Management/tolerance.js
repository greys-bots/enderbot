module.exports = {
	help: ()=> "Set the tolerance for globally or for a specific board.",
	usage: ()=> [
		" - Views and optionally clears global tolerance.",
		" [number] - Sets global tolerance.",
		" [channel] - Views and optionally clears current board tolerance.",
		" [channel] [number] - Sets board tolerance."
	],
		...
		switch(args.length) {
			...
			case 1:
				...
				if(!chan) { //assume this is setting the global tolerance
					...
					try {
						if(config) await bot.stores.configs.update(msg.guild.id, {tolerance: parseInt(args[0])});
						else await bot.stores.configs.create(msg.guild.id, {tolerance: parseInt(args[0])});
					} catch(e) {
						return "Error:\n"+e;
					}
